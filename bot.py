...
# ══════════════════════════════════════════════════════
#  🎯  VIEW : DRAFT — Cap2 pick ses 2 mates
# ══════════════════════════════════════════════════════
class DraftView(discord.ui.View):
    def __init__(self, match_id: int, cap2: int, pool: list[int]):
        ...
        self.done     = False
        for uid in pool:
            self.add_item(PickButton(uid=uid, parent=self))

class PickButton(discord.ui.Button):
    def __init__(self, uid: int, parent: DraftView):
        guild = bot.get_guild(GUILD_ID)
        m     = guild.get_member(uid) if guild else None
        label = m.display_name[:25] if m else str(uid)
        super().__init__(label=label, style=discord.ButtonStyle.blurple, custom_id=f"pick_{uid}")
        self.uid    = uid
        self.parent = parent
...
async def launch_match(players: list[int]):
    ...
    for uid in players:
        ...
        if m and m.voice:
            ...
            tp_ok.append(uid)  # pas en vocal, on garde quand même
    ...
    # Joueurs qui ont leave le lobby
    leaved = [uid for uid in players if uid not in in_lobby]

    if leaved:
        # Annulation : DM les leavers, remettre les autres en queue
        ...
        for uid in leaved:
            ...
            if m:
                try:
                    await m.send(f"❌ Tu as quitté le lobby — le match a été annulé.")
                except:
                    pass

        # Remettre les restants dans la queue vocale
        ...
        for uid in stayed:
            ...
            if m and m.voice:
                try:
                    ...
                    if uid not in queue:
                        queue.insert(0, uid)  # priorité en tête de queue
                except:
                    pass
...
async def start_draft(channel, match_id: int, cap1: int, cap2: int):
    ...
    await channel.send(
        ...
        f"🔴 **Cap 1** : {m1.mention} — pick automatique ✅\n"
        f"🔵 **Cap 2** : {m2.mention} — choisis tes **2 mates** ↓"
    )

    view = DraftView(match_id=match_id, cap2=cap2, pool=pool)
...
async def finalize_draft(channel, match_id: int):
    ...
    # TP chaque joueur dans son vocal
    for uid in team1:
        m = guild.get_member(uid)
        if m and m.voice:
...
async def process_result(channel, match_id: int, winner_team: str):
    ...
    # Elo moyen par équipe pour le calcul
...
async def update_leaderboard(guild: discord.Guild, data: dict):
    ...
    # Edit le dernier message du bot dans ce channel
...
async def on_voice_state_update(member: discord.Member, before: discord.VoiceState, after: discord.VoiceState):
    ...
    if after.channel and after.channel.id == QUEUE_VC_ID:
        ...
        if len(queue) >= QUEUE_SIZE:
            ...
            # Message dans le lobby : TP immédiat
            lobby_vc = guild.get_channel(LOBBY_VC_ID)
            for uid in players:
                m = guild.get_member(uid)
                if m and m.voice:
